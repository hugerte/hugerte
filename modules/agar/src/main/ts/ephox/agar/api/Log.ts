...
const steps = (testId: string, description: string, fs: Step<any, any>[]): Step<any, any>[] => fs.map((f, i) => step(testId, description + ' (' + i + ')', f));
...
const chains = (testId: string, description: string, cs: Chain<any, any>[]): Chain<any, any>[] => cs.map((c, i) => chain(testId, description + ' (' + i + ')', c));
