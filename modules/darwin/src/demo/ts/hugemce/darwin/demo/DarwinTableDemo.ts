...
import { Ephemera } from 'hugemce/darwin/api/Ephemera';
import * as InputHandlers from 'hugemce/darwin/api/InputHandlers';
import { SelectionAnnotation } from 'hugemce/darwin/api/SelectionAnnotation';
import * as SelectionKeys from 'hugemce/darwin/api/SelectionKeys';
import { Response } from 'hugemce/darwin/selection/Response';
import * as Util from 'hugemce/darwin/selection/Util';

const hugemceUi = SelectorFind.first('#hugemce-ui').getOrDie();
Attribute.set(hugemceUi, 'contenteditable', 'true');
...
const style = SugarElement.fromHtml<HTMLStyleElement>(
  ...
  'td.hugemce-darwin-selected { background: #cadbee; }\n' +
...
Insert.append(hugemceUi, rtlTable);
...
const mouseHandlers = InputHandlers.mouse(window, hugemceUi, Fun.curry(Compare.eq, table), annotations);
const keyHandlers = InputHandlers.keyboard(window, hugemceUi, Fun.curry(Compare.eq, table), annotations);

DomEvent.bind(hugemceUi, 'mousedown', mouseHandlers.mousedown);
DomEvent.bind(hugemceUi, 'mouseover', mouseHandlers.mouseover);
DomEvent.bind(hugemceUi, 'mouseup', mouseHandlers.mouseup);
...
DomEvent.bind(hugemceUi, 'keyup', (event) => {
...
DomEvent.bind(hugemceUi, 'keydown', (event) => {
