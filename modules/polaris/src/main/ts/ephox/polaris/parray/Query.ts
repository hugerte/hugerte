 ...
 */
const get = <T extends PRange>(parray: T[], offset: number): (T) | null => {
  return ((parray).find((x) => {
    return inUnit(x, offset);
  }) ?? null);
};

const startindex = (parray: PRange[], offset: number): (number) | null => {
  return (parray).findIndex((unit) => {
...
const sublist = <T extends PRange>(parray: T[], start: number, finish: number): T[] => {
  ...
  return first.bind((fIndex): (T[]) | null => {
    const last = rawlast ?? (tryend(parray, finish));
    return last > -1 ? parray.slice(fIndex, last) : null;
  }) ?? ([]);
};
