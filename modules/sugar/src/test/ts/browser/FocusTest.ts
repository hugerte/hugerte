...
UnitTest.test('Focus.active in ShadowRoot', () => {
  withShadowElement((sr, id, sh) => {
    ...
    Assert.eq('ShadowRoot\'s active element is the inner input box', Focus.active(sr), innerInput, tOptional(tElement()));
    Assert.eq('Document\'s active element is the shadow host', Focus.active(), sh, tOptional(tElement()));

    Focus.focus(outerInput);
    Assert.eq('ShadowRoot\'s active element should be none', Focus.active(sr), null, tOptional(tElement()));
    Assert.eq('Document\'s active element is the outer input box', Focus.active(), outerInput, tOptional(tElement()));
...
UnitTest.test('Focus.search in ShadowRoot', () => {
  withShadowElement((sr, id, sh) => {
    ...
    Focus.focus(innerInput);
    Assert.eq('Searching from div inside shadow root should yield focused input box', Focus.search(id), innerInput, tOptional(tElement()));
    Assert.eq('Searching from shadow root should yield focused input box', Focus.search(sr), innerInput, tOptional(tElement()));
    Assert.eq('Searching from shadow host should yield shadow host', Focus.search(sh), sh, tOptional(tElement()));
    Assert.eq('Searching from body should yield shadow host', Focus.search(SugarBody.body()), sh, tOptional(tElement()));

    Focus.focus(outerInput);
    Assert.eq('Searching from div inside shadow root should yield none', Focus.search(id), null, tOptional(tElement()));
    Assert.eq('Searching from shadow root should yield none', Focus.search(sr), null, tOptional(tElement()));
    Assert.eq('Searching from shadow host should yield none', Focus.search(sh), null, tOptional(tElement()));
    Assert.eq('Searching from body should yield outer input box', Focus.search(SugarBody.body()), outerInput, tOptional(tElement()));
