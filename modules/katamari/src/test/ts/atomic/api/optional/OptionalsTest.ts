...
describe('atomic.katamari.api.optional.OptionalsTest', () => {
  ...
  it('Optionals.cat of xs.map(Optional.some) should be xs', () => {
    fc.assert(fc.property(
      fc.array(fc.json()),
      (arr) => {
        const options = arr.map(Optional.some);
        const output = Optionals.cat(options);
  ...
  it('Optionals.cat of nones.concat(somes).concat(nones) should be somes', () => {
    fc.assert(fc.property(
      ...
      fc.array(fc.json()),
      (before, on, after) => {
        const beforeNones: Optional<string>[] = before.map(Optional.none);
        const afterNones = after.map(Optional.none);
        const onSomes = on.map(Optional.some);
