...
describe('atomic.katamari.api.optional.OptionalsSequenceTest', () => {
  ...
  it('Array of numbers', () => {
    fc.assert(fc.property(fc.array(fc.integer()), (n) => {
      const someNumbers = n.map((x) => Optional.some(x));
  ...
  it('all some', () => {
    fc.assert(fc.property(fc.array(fc.integer()), (n) =>
      assertOptional(Optionals.sequence<number>(n.map((x) => Optional.some(x))), Optionals.traverse<number, number>(n, (x) => Optional.some(x)))
    ));
  });
});
