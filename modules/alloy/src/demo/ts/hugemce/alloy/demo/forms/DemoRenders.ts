...
import { AlloySpec, RawDomSchema, SketchSpec } from 'hugemce/alloy/api/component/SpecTypes';
...
import { ItemDataTuple, ItemSpec, SeparatorItemSpec, WidgetItemSpec } from 'hugemce/alloy/ui/types/ItemTypes';
import { MenuSpec } from 'hugemce/alloy/ui/types/MenuTypes';
import { PartialMenuSpec, TieredMenuSpec } from 'hugemce/alloy/ui/types/TieredMenuTypes';
...
const choice = (choiceSpec: { value: string; text: string }): { dom: RawDomSchema; value: string } => {
  ...
  return {
    dom: DomFactory.fromHtml(
      '<span class="hugemce-pastry-independent-button" title="' + spec.text + '" style="display: flex;"></span>'
    ),
