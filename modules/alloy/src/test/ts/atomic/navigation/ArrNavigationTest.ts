...
UnitTest.test('ArrNavigationTest', () => {
  ...
  fc.assert(fc.property(arbTestCase, (testCase) => {
    ArrNavigation.cycleNext(testCase.values, testCase.index, () => true).getOrDie(
  ...
  fc.assert(fc.property(arbTestCase, (testCase) => {
    ArrNavigation.cycleNext(testCase.values, testCase.index, () => false).each((_) => {
  ...
  fc.assert(fc.property(arbUniqueNumTestCase, (testCase) => {
    ...
    const before = ArrNavigation.cyclePrev<number>(testCase.values, initial, () => true).getOrDie(
      'Should always be able to cycle prev on a >= 2 length array'
    );
    // Note, the index is the same as the value, so we can do this.
    const after = ArrNavigation.cycleNext(testCase.values, before, () => true).getOrDie(
      'Should always be able to cycle next on a >= 2 length array'
    );
  ...
  fc.property(arbUniqueNumTestCase, (testCase) => {
    const initial = testCase.index;
    const after = ArrNavigation.cycleNext<number>(testCase.values, initial, () => true).getOrDie(
      'Should always be able to cycle next on a >= 2 length array'
    );
    // Note, the index is the same as the value, so we can do this.
    const before = ArrNavigation.cyclePrev(testCase.values, after, () => true).getOrDie(
      'Should always be able to cycle prev on a >= 2 length array'
    );
  ...
  fc.assert(fc.property(arbUniqueNumTestCase, (testCase) => {
    const after = ArrNavigation.cycleNext(testCase.values, testCase.index, () => true).getOrDie(
      'Should always be able to cycle next on a >= 2 length array'
    );
  ...
  fc.assert(fc.property(arbUniqueNumTestCase, (testCase) => {
    const before = ArrNavigation.cyclePrev(testCase.values, testCase.index, () => true).getOrDie(
      'Should always be able to cycle prev on a >= 2 length array'
    );
  ...
  fc.assert(fc.property(arbUniqueNumTestCase, (testCase) => {
    ArrNavigation.tryNext(testCase.values, testCase.index, () => true).fold(
  ...
  fc.assert(fc.property(arbUniqueNumTestCase, (testCase) => {
    ArrNavigation.tryPrev(testCase.values, testCase.index, () => true).fold(
