...
interface SnapCandidate<E> {
  ...
  snap: (SnapConfig<E>) | null;
}

const findClosestSnap = <E>(component: AlloyComponent, snapInfo: SnapsConfig<E>, newCoord: DragCoord.CoordAdt, scroll: SugarPosition, origin: SugarPosition): (SnapOutput<E>) | null => {
  ...
  return matchSnap.orThunk((): (SnapOutput<E>) | null => {
    const bestSnap = (snaps).reduce((acc: SnapCandidate<E>, snap: SnapConfig<E>): SnapCandidate<E> => {
      const sensor = snap.sensor;
...
const findSnap = <E>(component: AlloyComponent, snapInfo: SnapsConfig<E>, newCoord: DragCoord.CoordAdt, scroll: SugarPosition, origin: SugarPosition): (SnapOutput<E>) | null => {
  // You need to pass in the absX and absY so that they can be used for things which only care about snapping one axis and keeping the other one.
  const snaps = snapInfo.getSnapPoints(component);
