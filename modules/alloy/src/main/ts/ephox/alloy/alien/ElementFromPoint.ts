...
const insideComponent = (component: AlloyComponent, x: number, y: number): (SugarElement<Element>) | null => {
  ...
  const hasValidRect = (node: SugarElement<Element>): boolean => {
    const elem = SugarNode.isText(node) ? Traverse.parentElement(node) : node;
