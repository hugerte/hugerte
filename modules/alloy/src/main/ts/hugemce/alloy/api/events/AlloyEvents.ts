...
import { TransformFind } from '@hugemce/sugar';
...
const abort = <T extends EventFormat>(name: string, predicate: EventAbortHandler<T>): AlloyEventKeyAndHandler<T> => {
  return {
    key: name,
...
const can = <T extends EventFormat>(name: string, predicate: EventCanHandler<T>): AlloyEventKeyAndHandler<T> => {
  return {
    key: name,
    value: EventHandler.nu({
      can: predicate
...
const preventDefault = <T extends EventFormat>(name: string): AlloyEventKeyAndHandler<T> => {
  return {
    key: name,
    value: EventHandler.nu({
...
const run = <T extends EventFormat>(name: string, handler: EventRunHandler<T>): AlloyEventKeyAndHandler<T> => {
  return {
    key: name,
    value: EventHandler.nu({
      run: handler
...
const runActionExtra = <T extends EventFormat>(name: string, action: (t: AlloyComponent, se: SimulatedEvent<T>, u: any) => void, extra: any[]): AlloyEventKeyAndHandler<T> => {
  return {
    key: name,
    value: EventHandler.nu({
      run: (component: AlloyComponent, simulatedEvent: SimulatedEvent<T>) => {
...
const runOnName = <T extends EventFormat>(name: string): RunOnName<T> => {
  return (handler) => run(name, handler);
};

const runOnSourceName = <T extends EventFormat> (name: string): RunOnSourceName<T> => {
...
const redirectToUid = <T extends EventFormat>(name: string, uid: string): AlloyEventKeyAndHandler<T> => {
...
const redirectToPart = <T extends EventFormat>(name: string, detail: CompositeSketchDetail, partName: string): AlloyEventKeyAndHandler<T> => {
  const uid = detail.partUids[partName];
  return redirectToUid(name, uid);
};

const runWithTarget = <T extends EventFormat>(name: string, f: (component: AlloyComponent, target: AlloyComponent, se: SimulatedEvent<T>) => void): AlloyEventKeyAndHandler<T> => {
...
const cutter = <T extends EventFormat>(name: string): AlloyEventKeyAndHandler<T> => {
  return run(name, (component, simulatedEvent) => {
    simulatedEvent.cut();
  });
};

const stopper = <T extends EventFormat>(name: string): AlloyEventKeyAndHandler<T> => {
  return run(name, (component, simulatedEvent) => {
    simulatedEvent.stop();
  });
};

const runOnSource = <T extends EventFormat>(name: string, f: EventRunHandler<T>): AlloyEventKeyAndHandler<T> => {
