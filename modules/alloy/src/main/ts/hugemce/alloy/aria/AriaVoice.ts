...
import { Attribute, Css, Insert, Remove, SugarElement, Traverse } from '@hugemce/sugar';
...
const tokenSelector = Fun.constant('span[id^="hugemce-alloy-aria-voice"]');
...
const linkToDescription = (item: SugarElement<Element>, token: SugarElement<Element>): void => {
  const id = Id.generate('hugemce-alloy-aria-voice');
