...
const all = (plugins, themes, models) => {
  return [
    create({
      ...
      ...buildEntries(plugins, 'plugins', 'Main.ts', 'js/hugerte/'),
      ...buildDemoEntries(models, 'models', 'Demo.ts', 'scratch/demos/'),
      ...buildEntries(models, 'models', 'Main.ts', 'js/hugerte/'),
      ...buildDemoEntries(themes, 'themes', 'Demos.ts', 'scratch/demos/'),
      ...buildEntries(themes, 'themes', 'Main.ts', 'js/hugerte/'),
    }, '../../tsconfig.demo.json'),
    // Note: This can't use the demo tsconfig as it is the core package
    create({
      'js/hugerte/hugerte.js': 'src/core/main/ts/api/Main.ts'
    }, '../../tsconfig.json')
  ];
};
