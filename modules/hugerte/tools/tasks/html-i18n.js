...
const resourceBase = 'hugerte.html-i18n';
...
const createHugeRTEJs = (translation, resourceName) =>
  `hugerte.Resource.add('${resourceName}',\n${util.inspect(translation)});`
...
const generate = (grunt, srcDir, outputDir, resourcePrefix) => {
  findFiles(srcDir).forEach((file) => {
    ...
    const content = createHugeRTEJs(translation, `${resourcePrefix}.${langCode}`);
