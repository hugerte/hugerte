 ...
 *
 * @example
 * const parser = hugerte.html.DomParser({ validate: true }, schema);
 * const rootNode = parser.parse('<h1>content</h1>');
 *
 * @class hugerte.html.DomParser
 * @version 3.4
 */
...
const DomParser = (settings: DomParserSettings = {}, schema = Schema()): DomParser => {
   ...
   * @return {hugerte.html.Node} Root node containing the tree.
   * @example
   * const rootNode = hugerte.html.DomParser({...}).parse('<b>text</b>');
   */
