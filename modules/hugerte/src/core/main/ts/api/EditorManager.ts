...
declare const window: Window & { hugerte?: PreInit; hugeRTEPreInit?: PreInit };
...
const EditorManager: EditorManager = {
  ...
  /**
   * Major version of HugeRTE build.
   *
  ...
  /**
   * Minor version of HugeRTE build.
   *
   ...
   * @type hugerte.Editor
   * @example
   * hugeRTE.activeEditor.selection.getContent();
   * hugerte.EditorManager.activeEditor.selection.getContent();
   */
  activeEditor: null,
  focusedEditor: null,
  ...
  setup() {
    ...
    // If hugerte is defined and has a base use that or use the old hugeRTEPreInit
    const preInit = window.hugerte || window.hugeRTEPreInit;
    if (preInit) {
      baseURL = preInit.base || preInit.baseURL;
      suffix = preInit.suffix;
    } else {
      // Get base where the hugerte script is located
    ...
    /**
     * Base URL where the root directory if HugeRTE is located.
     *
     * @property baseURL
     * @type String
     */
    ...
    /**
     * Absolute baseURI for the installation path of HugeRTE.
     *
     * @property baseURI
   ...
   * <br /><br />
   * Therefore, when using <code>overrideDefaults</code> with the cloud-based editor, any newly integrated options must be combined with the options in <code>hugerte.defaultOptions</code>.
   *
   ...
   * };
   *
   * hugerte.overrideDefaults({
   *   ...hugerte.defaultOptions,
   *   ...customOptions
   * });
   */
   ...
   * <br /><br />
   * For information on basic usage of <code>init</code>, see: <a href="https://www.tiny.cloud/docs/hugerte/6/basic-setup/">Basic setup</a>.
   *
   * @method init
    ...
    const findTargets = (options: RawEditorOptions): HTMLElement[] => {
      if (Env.browser.isIE() || Env.browser.isEdge()) {
        ErrorReporter.initError(
          ...
          ' browsers please see: https://www.tiny.cloud/docs/hugerte/6/support/#supportedwebbrowsers'
        );
        return [];
   ...
   * @return {hugerte.Editor/Array} Editor instance or an array of editor instances.
   * @example
   * // Adds an onclick event to an editor by id
   * hugerte.get('mytextbox').on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   *
   * // Adds an onclick event to an editor by index
   * hugerte.get(0).on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   *
   * // Adds an onclick event to an editor by id (longer version)
   * hugerte.EditorManager.get('mytextbox').on('click', (e) => {
   *   ed.windowManager.alert('Hello world!');
   * });
   */
   ...
   *
   * @method add
   * @param {hugerte.Editor} editor Editor instance to add to the collection.
   * @return {hugerte.Editor} The same instance that got passed in.
   */
  add(editor) {
    const self: EditorManager = this;
   ...
   * @return {hugerte.Editor} Editor instance that got created.
   */
   ...
   *
   * @method remove
   * @param {hugerte.Editor/String/Object} [selector] CSS selector or editor instance to remove.
   * @return {hugerte.Editor} The editor that got passed in will be return if it was found otherwise null.
   */
   ...
   *
   * @method setActive
   * @param {hugerte.Editor} editor Editor instance to set as the active instance.
   */
  setActive(editor) {
