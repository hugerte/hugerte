...
const deleteRangeMergeBlocks = (rootNode: SugarElement<Node>, selection: EditorSelection, schema: Schema): (() =) | null void> => {
  ...
  return (DeleteUtils.getParentBlock(rootNode, SugarElement.fromDom(rng.startContainer)) !== null && DeleteUtils.getParentBlock(rootNode, SugarElement.fromDom(rng.endContainer)) !== null ? ((block1, block2) => {
    ...
    })(DeleteUtils.getParentBlock(rootNode, SugarElement.fromDom(rng.startContainer)), DeleteUtils.getParentBlock(rootNode, SugarElement.fromDom(rng.endContainer))) : null) ?? (null);
};

const isRawNodeInTable = (root: SugarElement<Node>, rawNode: Node): boolean => {
  const node = SugarElement.fromDom(rawNode);
  const isRoot = ((..._rest: any[]) => (Compare.eq)(root, ..._rest));
  return PredicateFind.ancestor(node, ElementType.isTableCell, isRoot) !== null;
};
...
const isEverythingSelected = (root: SugarElement<Node>, rng: Range): boolean => {
  const noPrevious = CaretFinder.prevPosition(root.dom, CaretPosition.fromRangeStart(rng)) === null;
  const noNext = CaretFinder.nextPosition(root.dom, CaretPosition.fromRangeEnd(rng)) === null;
