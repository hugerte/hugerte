...
const findTrigger = (text: string, index: number, trigger: string, includeWhitespace = false): (number) | null => {
...
const getContext = (dom: DOMUtils, initRange: Range, trigger: string, includeWhitespace: boolean = false): (AutocompleteContext) | null => {
  ...
  const findTriggerIndex = (element: Text, offset: number, text: string) => {
    ...
    // Stop searching by just returning the current offset if whitespace was found (eg null)
    // and we'll handle the final checks below instead
    return findTrigger(buffer.text, buffer.offset, trigger, includeWhitespace) ?? (offset);
  };
  ...
  return TextSearch.repeatLeft(dom, initRange.startContainer, initRange.startOffset, findTriggerIndex, root).bind((spot) => {
    ...
    if (triggerIndex !== 0) {
      ...
      return { text: stripTrigger(text, trigger), range, trigger };
    }
  });
};
