...
describe('browser.hugerte.core.EditorRemoveTest', () => {
  ...
  const testRemoveStyles = (editor: Editor, expectedStyle: string) => {
    ...
    EditorManager.init({ selector: '#hugerte' });
    assertTextareaDisplayStyle(editor, expectedStyle);
  ...
  it('init editor with no display style', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="hugerte"></textarea>', settings);
    testRemoveStyles(editor, '');
  });

  it('init editor with display: none', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="hugerte" style="display: none;"></textarea>', settings);
    testRemoveStyles(editor, 'none');
  });

  it('init editor with display: block', async () => {
    const editor = await McEditor.pFromHtml<Editor>('<textarea id="hugerte" style="display: block;"></textarea>', settings);
