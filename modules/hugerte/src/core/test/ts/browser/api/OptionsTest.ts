...
describe('browser.hugerte.core.api.OptionsTest', () => {
  it('getBodyId', () => {
    assert.equal(Options.getBodyId(new Editor('id', {}, EditorManager)), 'hugerte', 'Should be default id');
    ...
    assert.equal(Options.getBodyId(new Editor('idc', { body_id: 'ida=a,idb=b' }, EditorManager)), 'hugerte', 'Should be default id for idc');
  });
