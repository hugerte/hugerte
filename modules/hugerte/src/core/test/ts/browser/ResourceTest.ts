...
describe('browser.hugerte.core.ResourceTest', () => {
  ...
  const addScript = (id: string, data: string) => {
    hugerte.Resource.add(id, data);
  };

  const loadScript = (id: string, url: string): Promise<any> =>
    hugerte.Resource.load(id, url);

  const unloadScript = (id: string) => {
    hugerte.Resource.unload(id);
  };
  ...
  it('invalid id fails', () => {
    ...
    return assertLoadFailure(load, `Script at URL "data:text/javascript,hugerte.Resource.add('invalid-id', 'value.4')" did not call \`hugerte.Resource.add('script.4', data)\` within 1 second`);
  });
