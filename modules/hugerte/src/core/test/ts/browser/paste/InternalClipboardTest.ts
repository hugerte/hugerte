...
describe('browser.hugerte.core.paste.InternalClipboardTest', () => {
  ...
  context('copy', () => {
    it('TBA: Copy simple text', () => {
      ...
      assertClipboardData('<!-- x-hugerte/html -->text', 'text');
    ...
    it('TBA: Copy inline elements', () => {
      ...
      assertClipboardData('<!-- x-hugerte/html -->te<em>x</em>t', 'text');
    ...
    it('TBA: Copy partially selected inline elements', () => {
      ...
      assertClipboardData('<!-- x-hugerte/html -->a<em>c</em>', 'ac');
    ...
    it('TINY-8563: Copy cef element', () => {
      ...
      assertClipboardData('<!-- x-hugerte/html --><span contenteditable="false">bc</span>', 'bc');
  ...
  context('paste', () => {
    ...
    it('TBA: Paste internal content with mime', async () => {
      const editor = hook.editor();
      paste(editor, '<p>abc</p>', { 'text/plain': 'X', 'text/html': '<p>X</p>', 'x-hugerte/html': '<p>X</p>' }, [ 0, 0 ], 0, [ 0, 0 ], 3);
      await pWaitForAndAssertEvents({ internal: true, content: '<p>X</p>' }, '<p>X</p>');
    });
