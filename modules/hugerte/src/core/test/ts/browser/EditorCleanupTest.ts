...
import VisualBlocksPlugin from 'hugerte/plugins/visualblocks/Plugin';

describe('browser.hugerte.core.EditorCleanupTest', () => {
  before(() => {
    VisualBlocksPlugin();
  });
  ...
  testCleanup(
    ...
    '<div></div>',
    (editor) => {
      editor.dom.loadCSS('/project/hugerte/js/hugerte/skins/ui/dark/skin.css');
    });

  // Loaded via StyleSheetLoader
  assertPageLinkPresence('/project/hugerte/js/hugerte/skins/ui/content/default/content.inline.css', false);
  assertPageLinkPresence('/project/hugerte/js/hugerte/skins/ui/oxide/skin.css', false);
  // Loaded via DOMUtils as per above
  assertPageLinkPresence('/project/hugerte/js/hugerte/skins/ui/dark/skin.css', false);
});
