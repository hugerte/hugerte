...
import * as EditorFocus from 'hugerte/core/focus/EditorFocus';

describe('browser.hugerte.core.focus.EditorFocusTest', () => {
  ...
  context('Focus editor', () => {
    it('Focus editor initialized on a div with p', async () => {
      const editor = await pCreateInlineEditor('<div class="hugerte-editor"><p>a</p></div>');
      focusEditor(editor);
    ...
    it('Focus editor initialized on a list', async () => {
      const editor = await pCreateInlineEditor('<ul class="hugerte-editor"><li>a</li></ul>');
      focusEditor(editor);
      TinyAssertions.assertSelection(editor, [ 0, 0 ], 0, [ 0, 0 ], 0);
      McEditor.remove(editor);
    });
    ...
    it('Selection restored on focus with table cE=true', async () => {
      const editor = await pCreateEditor('<div class="hugerte-editor"><p>a</p><div>b<table contenteditable="true"><tbody><tr><td>c</td><td></td></tr></tbody></table></div></div>');
  ...
  context('hasFocus', () => {
    ...
    it('Focus on cE=true inside a cE=false', async () => {
      const editor = await pCreateInlineEditor('<div class="hugerte-editor"><div contenteditable="false">a<div contenteditable="true">b</div></div></div>');
