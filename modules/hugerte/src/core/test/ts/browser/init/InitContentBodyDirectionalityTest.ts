...
describe('browser.hugerte.core.init.InitContentBodyDirectionalityTest', () => {
  before(() => {
  ...
  const runTest = async (settings: RawEditorOptions, label: string, expected: string) => {
    const editor = await McEditor.pFromSettings<Editor>({ base_url: '/project/hugerte/js/hugerte', ...settings });
