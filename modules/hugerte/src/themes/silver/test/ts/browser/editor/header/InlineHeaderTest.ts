...
describe('browser.hugerte.themes.silver.editor.header.InlineHeaderTest', () => {
  ...
  const pAssertHeaderWidth = (expectedWidth: string, expectedMaxWidth: string) =>
    Waiter.pTryUntil('Could not verify width', () => {
      ...
      const headerWrapper = SelectorFind.descendant(SugarBody.body(), '.tox-hugerte--toolbar-sticky-off').getOrDie();
