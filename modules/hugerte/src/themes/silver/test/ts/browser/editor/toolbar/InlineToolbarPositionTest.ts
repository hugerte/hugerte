...
describe.skip('browser.hugerte.themes.silver.editor.toolbar.InlineToolbarPositionTest', () => {
  const options = {
    inline: true,
    menubar: false,
  ...
  const pDeactivateEditor = async (editor: Editor) => {
    ...
    await UiFinder.pWaitForHidden('Wait for editor to hide', SugarBody.body(), '.tox.tox-hugerte-inline');
  };
