...
import { Focus, Insert, Remove, SugarBody, SugarElement } from "@hugerte/sugar";
import { McEditor, TinySelections } from "@hugerte/wrap-mcagar";

import Editor from 'hugerte/core/api/Editor';

// TODO TINY-10480: Investigate flaky tests
describe.skip('browser.hugerte.themes.silver.editor.contexttoolbar.RemoveContextToolbarOnFocusoutTest', () => {
  ...
  it('iframe editor focusout should remove context toolbar', async () => {
    const editor = await McEditor.pFromHtml<Editor>(html, { setup, base_url: '/project/hugerte/js/hugerte' });
    editor.focus();
  ...
  it('inline editor focusout should remove context toolbar', async () => {
    const editor = await McEditor.pFromHtml<Editor>(html, { setup, inline: true, base_url: '/project/hugerte/js/hugerte' });
    editor.focus();
