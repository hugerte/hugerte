...
export interface FormatterFormatItem {
  ...
  readonly isSelected: (value: (any) | null) => boolean;
  readonly getStylePreview: () => (PreviewSpec) | null;
}
...
export interface SelectSpec {
  ...
  readonly getCurrentValue: () => (SelectedFormat) | null;
...
interface BespokeMenuItems {
  readonly items: {
    ...
    readonly getFetch: (backstage: UiFactoryBackstage, getStyleItems: () => FormatItem[]) => (comp: AlloyComponent, callback: (menu: (TieredData) | null) => void) => void;
  };
  readonly getStyleItems: () => FormatItem[];
}
...
const generateSelectItems = (backstage: UiFactoryBackstage, spec: SelectSpec) => {
  ...
  const validate = (item: FormatItem, response: IrrelevantStyleItemResponse, value: (SelectedFormat) | null): Menu.NestedMenuItemContents[] => {
  ...
  const validateItems = (preItems: FormatItem[]) => {
    ...
    return (preItems).flatMap((item) => validate(item, response, value));
  };

  const getFetch = (backstage: UiFactoryBackstage, getStyleItems: () => FormatItem[]) => (comp: AlloyComponent, callback: (menu: (TieredData) | null) => void) => {
...
const createMenuItems = (backstage: UiFactoryBackstage, spec: SelectSpec): BespokeMenuItems => {
  ...
  const getStyleItems = dataset.type === 'basic' ?
    () => (dataset.data).map((d) => FormatRegister.processBasic(d, spec.isSelectedFor, spec.getPreviewFor)) :
    dataset.getData;
  return {
...
const createSelectButton = (editor: Editor, backstage: UiFactoryBackstage, spec: SelectSpec, tooltipWithPlaceholder: string, textUpdateEventName: string, btnName: string): SketchSpec => {
  ...
  return renderCommonDropdown(
    {
      text: spec.icon !== null ? null : spec.text,
      icon: spec.icon,
      ariaLabel: spec.tooltip,
      tooltip: null, // TINY-10474 - Using own tooltip config
      role: null,
      ...
      columns: 1,
      presets: 'normal',
      classes: spec.icon !== null ? [] : [ 'bespoke' ],
      dropdownBehaviours: [
        Tooltipping.config({
