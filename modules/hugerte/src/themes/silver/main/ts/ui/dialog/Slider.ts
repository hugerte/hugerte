...
export const renderSlider = (spec: SliderSpec, providerBackstage: UiFactoryBackstageProviders, initialData: (number) | null): SimpleSpec => {
  ...
  return Slider.sketch({
    ...
    model: {
      ...
      minX: spec.min,
      maxX: spec.max,
      getInitialValue: () => initialData.getOrThunk(() => (Math.abs(spec.max) - Math.abs(spec.min)) / 2)
    },
    components: [
      labelPart,
      spectrum,
