...
describe('browser.hugerte.plugins.link.UpdateLinkTest', () => {
  ...
  it('TBA: should not get anchor info if not selected node', async () => {
    ...
    editor.setContent('<p><a href="http://hugerte.com" class="shouldbekept" title="shouldalsobekept">tiny</a></p>');
    TinySelections.setCursor(editor, [ 0, 0, 0 ], 2);
    editor.execCommand('mcelink');
    await TinyUiActions.pWaitForDialog(editor);
    TestLinkUi.assertDialogContents({
      href: 'http://hugerte.com',
      text: 'tiny',
  ...
  it('TBA: should remove attributes if unset in the dialog', async () => {
    const editor = hook.editor();
    editor.setContent('<p><a href="http://hugerte.com" class="shouldbekept" title="shouldnotbekept">tiny</a></p>');
    TinySelections.setCursor(editor, [ 0, 0, 0 ], 2);
    editor.execCommand('mcelink');
    await TinyUiActions.pWaitForDialog(editor);
    TestLinkUi.assertDialogContents({
  ...
  it('TINY-7998: Updating a link with a dangerous URL should remove the href attribute', async () => {
    const editor = hook.editor();
    editor.setContent('<p><a href="https://hugerte.com" title="shouldbekept">tiny</a></p>');
    TinySelections.setCursor(editor, [ 0, 0, 0 ], 2);
    editor.execCommand('mceLink');
    await TinyUiActions.pWaitForDialog(editor);
