...
import { NotificationApi } from 'hugerte/core/api/NotificationManager';
import FullscreenPlugin from 'hugerte/plugins/fullscreen/Plugin';
import LinkPlugin from 'hugerte/plugins/link/Plugin';

describe('browser.hugerte.plugins.fullscreen.FullScreenPluginTest', () => {
  ...
  const assertEditorContainerAndSinkState = (editor: Editor, shouldExist: boolean) => {
    ...
    const sink = UiFinder.findIn(contentContainer, '.tox-silver-sink.tox-hugerte-aux').getOrDie();
