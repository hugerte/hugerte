...
import { Attribute, Html, Insert, Remove, SelectorFind, SugarBody, SugarElement } from "@hugerte/sugar";
...
describe.skip('browser.hugerte.plugins.table.InlineEditorInsideTableTest', () => {
  const setupElement = () => {
    const containerHtml = '<table>' +
      '<tbody>' +
      '<tr>' +
      '<td>' +
      '<div class="hugerte" style="border: 1px gray solid">a</div>' +
      '</td>' +
      '</tr>' +
      '</tbody>' +
      '</table>';
    ...
    const editorDiv = SelectorFind.descendant<HTMLDivElement>(container, 'div.hugerte').getOrDie('Cannot find HugeRTE div');
    return {
      element: editorDiv,
