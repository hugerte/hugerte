...
import { Entry, isEntryList } from 'hugerte/plugins/lists/listmodel/Entry';
import { normalizeEntries } from 'hugerte/plugins/lists/listmodel/NormalizeEntries';
import { parseLists } from 'hugerte/plugins/lists/listmodel/ParseLists';
import { ListType } from 'hugerte/plugins/lists/listmodel/Util';

describe('browser.hugerte.plugins.lists.ListModelTest', () => {
  it('TBA: Validate lists plugin model', () => {
    ...
    const stringifyEntries = (entries: Entry[]): string => entries.map(stringifyEntry).join(',');
