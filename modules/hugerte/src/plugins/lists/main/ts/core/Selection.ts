...
const isParentListSelected = (parentList: HTMLElement | null, selectedBlocks: Element[]): boolean =>
  (parentList) != null && selectedBlocks.length === 1 && selectedBlocks[0] === parentList;

const findSubLists = (parentList: HTMLElement): HTMLElement[] =>
  (parentList.querySelectorAll(listSelector)).filter(NodeType.isListNode);
...
const getSelectedSubLists = (editor: Editor): HTMLElement[] => {
  ...
  if (isParentListSelected(parentList, selectedBlocks)) {
    ...
    return (selectedBlocks).filter((elm): elm is HTMLElement => {
...
const getSelectedListItems = (editor: Editor): Array<HTMLLIElement | HTMLElement> => {
  const selectedBlocks = editor.selection.getSelectedBlocks();
  return (findParentListItemsNodes(editor, selectedBlocks)).filter(NodeType.isListItemNode);
};

const getSelectedDlItems = (editor: Editor): HTMLElement[] =>
  (getSelectedListItems(editor)).filter(NodeType.isDlItemNode);
...
const isListHost = (schema: Schema, node: Node): boolean =>
  !NodeType.isListNode(node) && !NodeType.isListItemNode(node) && (listNames).some((listName) => schema.isValidChild(node.nodeName, listName));
...
const getClosestListHost = (editor: Editor, elm: Node): HTMLElement => {
  ...
  const parentBlock = ((parentBlocks).find((elm) => isListHost(editor.schema, elm)) ?? null);
...
const findLastParentListNode = (editor: Editor, elm: Element): (HTMLOListElement | HTMLUListElement) | null => {
  const parentLists = editor.dom.getParents<HTMLOListElement | HTMLUListElement>(elm, 'ol,ul', getClosestListHost(editor, elm));
  return ((parentLists).at(-1) ?? null);
};

const getSelectedLists = (editor: Editor): Array<HTMLOListElement | HTMLUListElement> => {
  const firstList = findLastParentListNode(editor, editor.selection.getStart());
  const subsequentLists = (editor.selection.getSelectedBlocks()).filter(NodeType.isOlUlNode);
...
const getSelectedListRoots = (editor: Editor): HTMLElement[] => {
  ...
  return ((parentLists).find((p) => isListInsideAnLiWithFirstAndLastNotListElement(SugarElement.fromDom(p))) ?? null).fold(
...
const getUniqueListRoots = (editor: Editor, lists: HTMLElement[]): HTMLElement[] => {
  const listRoots = (lists).map((list) => findLastParentListNode(editor, list) ?? (list));
