...
const point = <E, D>(universe: Universe<E, D>, start: E, soffset: number, _finish: E, _foffset: number, exclusions: (e: E) => boolean): (SpanWrapRange<E>) | null => {
...
const temporary = <E, D>(universe: Universe<E, D>, start: E, soffset: number): SpanWrapPoint<E> => {
  ...
  const cursor = universe.create().text('\uFEFF', doc);
  universe.insert().append(span, cursor);

  const injectAt = universe.property().isEmptyTag(start) ? universe.property().parent(start) : start;
  injectAt.each((z) => {
...
const isUnicode = <E, D>(universe: Universe<E, D>, element: E): boolean => {
  return universe.property().isText(element) && universe.property().getText(element) === '\uFEFF';
};
...
const wrap = <E, D>(universe: Universe<E, D>, start: E, soffset: number, finish: E, foffset: number, exclusions: (e: E) => boolean): (SpanWrapRange<E>) | null => {
  ...
  return (wrappers[wrappers.length - 1] ?? null).map((lastSpan): SpanWrapRange<E> => {
...
const spans = <E, D>(universe: Universe<E, D>, start: E, soffset: number, finish: E, foffset: number, exclusions: (e: E) => boolean = (() => false as const)): (SpanWrapRange<E>) | null => {
